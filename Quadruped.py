...
def mutate(gen):
    ...
    if (index == 16) :
        index = 15
    if ((index <8 )and(index>=4)) or(index<16 and index>=12) :
...
def convert(genome):
    i=0
    while i < 32:
        a=((((genome[i] - -70) * (0.7 - -0.7)) / (70 - -70)) + -0.7)
...
generation =1
genome=initgenome()
cgenome=convert(genome)    
best_genome=cgenome[:]
for j in range (0,5000):
        print("Generation:")
        print(generation)
        cur_genome=mutate(best_genome[:])
        print("Genome:")
        print(cur_genome)
        vrep.simxLoadModel(clientID,"C:\quad0_1.ttm",0,vrep.simx_opmode_blocking)
        time.sleep(1)
        ...
        while k <=10:
                ...
                time.sleep(1)
                k=k+1
        returnCode,Data=vrep.simxGetObjectPosition(clientID,model_handle,-1,vrep.simx_opmode_blocking)
        print("Position:")
        print(Data)
        cur_fitness=fitness(Data)
        print("Fitness:")
        print(cur_fitness)
        fitness_track.append(cur_fitness)
